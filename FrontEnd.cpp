...
void cGameFront::mControls()
{
	...
	changeKeyText.xPos = 400;
	changeKeyText.yPos = 650;

	textData cancelKeyChange;
	cancelKeyChange.text << "Cancel";
	cancelKeyChange.xPos = 1000;
	cancelKeyChange.yPos = 650;

	string newKeyChar;

	//X Positions
	controlText[0].xPos = (windowWidth / 2) - 100;
	controlText[1].xPos = (windowWidth / 2) - 100;
	controlText[2].xPos = (windowWidth / 2) - 100;
	controlText[3].xPos = (windowWidth / 2) - 100;
	controlText[4].xPos = (windowWidth / 2) - 100;
	//Y Positions
	controlText[0].yPos = 250;
	controlText[1].yPos = 300;
	controlText[2].yPos = 350;
	controlText[3].yPos = 400;
	controlText[4].yPos = 450;
...
void cGameFront::mShowMaps()
{
	...
	mSceneMusic->mStop();
	cGameInstance* newGame = new cGameInstance(mFrontEngine, "Demo.txt", gameControl);
	mMenuPanel->MoveY(1000.0f);
	mSceneMusic->mStart();
